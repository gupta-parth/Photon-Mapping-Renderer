...
public:
	...
	Vertex getScreenPoint(const float3& vertex, const float4x4& plm) const {
		float4 clipPt = mul(plm, float4{vertex.x, vertex.y, vertex.z, 1.0f});
		// Check here if pt needs to be clipped or not?
		float W = 1.0f / clipPt.w;
		float3 ndcPt = {
			clipPt.x * W, clipPt.y * W, clipPt.z * W
		};
		float screen_x = (ndcPt.x + 1.0f) * 0.5 * globalWidth;
		float screen_y = (ndcPt.y + 1.0f) * 0.5 * globalHeight;
		return {
			{screen_x, screen_y},
			ndcPt.z,
			W
	...
	bool raytraceTriangle(HitInfo& result, const Ray& ray, const Triangle& tri, float tMin, float tMax) const {
		...
		float3 ab = tri.positions[1] - tri.positions[0];
		float3 ac = tri.positions[2] - tri.positions[0];
		float3 dirCrossAC = cross(ray.d, ac);
		float det = dot(ab, dirCrossAC);
		float invDet = 1.0f / det;

		float3 aOrig = ray.o - tri.positions[0];

		// compute barycentric coordinates
		float beta = dot(aOrig, dirCrossAC) * invDet;
		if (beta < 0.0f || beta > 1.0f) return false;

		float3 aOrigCrossAB = cross(aOrig, ab);
		float gamma = dot(ray.d, aOrigCrossAB) * invDet;
		if (gamma < 0.0f || gamma + beta > 1.0f) return false;

		float t = dot(ac, aOrigCrossAB) * invDet;
